...
DynamicStr dynamic_str_init(char* str)
{
    ...
    if(new_str->str == NULL)
    {
        fprintf(stderr, "failed to allocate memory for the string inside the dynamic str\n");
        free(new_str);
        return NULL;
    }
...
int dynamic_str_cat(DynamicStr dest, char* src)
{
    ...
    {
        fprintf(stderr, "destination or src cannot be null when concatinting\n");
        return DYNAMIC_STR_ERROR;
    }

    dest->str = (char*)realloc(dest->str, sizeof(char) * (dest->size + strlen(src)));
    if(dest->str == NULL)
    {
        fprintf(stderr, "error occured while reallcoating memory to dynamic string\n");
        return DYNAMIC_STR_ERROR;
    }
...
int dynamic_str_change(DynamicStr dest, char* src)
{
    if(dest == NULL || src == NULL)
    {
        fprintf(stderr, "destination or src cannot be null when changing string\n");
        return DYNAMIC_STR_ERROR;
    }
...
int dynamic_str_remove(DynamicStr str, int index)
{
    ...
    {
        fprintf(stderr, "invalid index %d should be in range 0-%d\n", index, str->size - 1);
        return DYNAMIC_STR_ERROR;
    }
